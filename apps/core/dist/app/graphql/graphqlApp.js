...
function default_1({ 'core.depsManager': depsManager = null, 'core.domain.eventsManager': eventsManagerDomain = null, 'core.utils': utils = null, config = null } = {}) {
    let _fullSchema;
    const _pluginsSchema = [];
    const schemaUpdateEmitter = new events_1.EventEmitter();
    const SCHEMA_UPDATE_EVENT = 'schemaUpdate';
    const _generateSchema = async () => {
        try {
            ...
            for (const modName of modules) {
                ...
                if (typeof appModule.getGraphQLSchema === 'function') {
                    ...
                    appSchema.resolvers = (0, lodash_1.merge)(appSchema.resolvers, schemaToAdd.resolvers);
                }
            }
            for (const schemaPart of _pluginsSchema) {
                appSchema.typeDefs.push(schemaPart.typeDefs);
                appSchema.resolvers = (0, lodash_1.merge)(appSchema.resolvers, schemaPart.resolvers);
            }
            // Put together a schema
            const schema = (0, apollo_server_1.makeExecutableSchema)(appSchema);
            _fullSchema = schema;
    ...
    return {
        ...
        async initSchemaUpdateConsumer() {
            ...
            await eventsManagerDomain.initCustomConsumer('schema_update_events', config.eventsManager.routingKeys.data_events, async (msg, channel) => {
                channel.ack(msg);
                ...
                const handledActions = [
                    event_1.EventAction.LIBRARY_SAVE,
                    event_1.EventAction.LIBRARY_DELETE,
                    event_1.EventAction.ATTRIBUTE_SAVE,
                    event_1.EventAction.ATTRIBUTE_DELETE
                ];
        ...
        extensionPoints: {
            registerGraphQLSchema: (schemaPart) => {
